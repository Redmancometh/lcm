...
public class orker extends Observable implements Runnable {
	...
	private volatile ork curWork = null;
	private URL lpUrl = null;
	private HttpURLConnection lpConn = null;
	private AtomicLong hashes = new AtomicLong(0L);

	public orker(URL url, String auth, long scanMillis, long pauseMillis) {
		this(url, auth, scanMillis, pauseMillis, Runtime.getRuntime().availableProcessors());
	}

	public orker(URL url, String auth, long scanMillis, long pauseMillis, int nThreads) {
		this(url, auth, scanMillis, pauseMillis, nThreads, 1.0);
	}

	public orker(URL url, String auth, long scanMillis, long pauseMillis, int nThreads, double throttle) {
		this.url = url;
		this.auth = auth;
	...
	private synchronized ork getWork() {
		while (running) {
			try {
				return new ork(url, auth);
			} catch (Exception e) {
				if (!running)
					break;
        		setChanged();
		...
		public void run() {
			while (running) {
				try {
					...
					curWork = new ork(lpConn, url, auth);
					if (!running)
						break;
	        		synchronized (orker.this) {
			    		setChanged();
	...
	private class WorkSubmitter implements Runnable {
		private ork work;
		private int nonce;
		public WorkSubmitter(ork w, int nonce) {
			this.work = w;
			this.nonce = nonce;
		}
		public void run() {
